...
All component imports ( Header, PrimaryButton, LogoGrid, Card, CTAContainer) point to the paths we defined in the system‑token and component blueprints ( src/design_system/...).
Tailwind utilities ( bg-primaryDark, container, grid, etc.) map directly to the design tokens we created ( colors.primaryDark, spacing.64, etc.).
Replace placeholder SVG/PNG asset paths ( /assets/...) with the final exported assets from Figma (e.g., usps-1.svg, experience-detail.svg, selected-work-bg.png, etc.).
---
# Experience Page
